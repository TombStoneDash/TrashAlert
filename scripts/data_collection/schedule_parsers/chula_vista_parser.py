"""
Chula Vista trash schedule parser.

Extracts trash collection schedules for Chula Vista, CA.
Data source: Republic Services (city contractor)
"""
...
class ChulaVistaParser(BaseScheduleParser):
    """
    Parser for Chula Vista trash schedules.

    Chula Vista uses Republic Services for garbage collection.
    Trash is collected between 7 AM and 6 PM on service days.
    Containers should be placed at curb by 7 AM.
    """

    def __init__(self):
        super().__init__(
            city="Chula Vista",
            source_url="https://www.chulavistaca.gov/departments/clean/environmental-services/trash"
        )
        # Chula Vista has multiple collection routes/zones
        self.zones = {
            ...
            "ZONE_5": {"trash": "FRI", "recycling": "TUE", "green_waste": "FRI"},
        }

    def fetch_raw_data(self) -> Any:
        """
        Fetch schedule data from Chula Vista sources.

        For pilot version, use hardcoded zone data.
        In production, this would query Republic Services API or scrape website.
        """
        logger.info(f"Fetching data from {self.source_url}")

        # Republic Services holiday schedule
        # If scheduled service day falls on or after a holiday, service may be delayed by one day
        return {
            "zones": self.zones,
            "holidays": [
    ...
    def parse_raw_data(self, raw_data: Any) -> ParseResult:
        """
        Parse Chula Vista schedule data.
        ...
        try:
            ...
            for zone_name, zone_schedule in zones.items():
                for collection_type, day in zone_schedule.items():
                    schedule = ScheduleData(
                        address=f"Chula Vista, {zone_name}",
                        day_of_week=day,
            ...
            for holiday in holidays:
                try:
                    ...
                    exception = ExceptionData(
                        ...
                        notes=f"Chula Vista city holiday: {holiday.get('name', 'Unknown')}"
                    )
            ...
            result.errors.append(f"Failed to parse Chula Vista data: {e}")
    ...
    def match_address_to_zone(self, address: str) -> str:
        ...
        In production, would use Republic Services API or GIS polygon matching.
        ...
        if any(kw in address_upper for kw in ["BROADWAY", "THIRD", "DOWNTOWN"]):
            return "ZONE_1"
        elif any(kw in address_upper for kw in ["EASTLAKE", "OTAY", "OLYMPIC"]):
            return "ZONE_2"
        elif any(kw in address_upper for kw in ["BONITA", "TELEGRAPH", "HILLTOP"]):
            return "ZONE_3"
        elif any(kw in address_upper for kw in ["RANCHO DEL REY", "SANTA VENETIA"]):
            return "ZONE_4"
        else:
            return "ZONE_5"
