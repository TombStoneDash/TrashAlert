"""
Oceanside trash schedule parser.

Extracts trash collection schedules for Oceanside, CA.
Data source: Waste Management (WM) - city's franchise service provider
"""
...
class OceansideParser(BaseScheduleParser):
    """
    Parser for Oceanside trash schedules.

    Oceanside uses Waste Management (WM) as the franchise service provider.
    Collection starts at 6 AM. Residents have 3 carts: blue (recycling), green (organics), black/gray (trash).
    """

    def __init__(self):
        super().__init__(
            city="Oceanside",
            source_url="https://www.ci.oceanside.ca.us/government/water-utilities/environmental-services-programs"
        )
        # Oceanside has multiple collection routes/zones
        self.zones = {
            ...
            "ZONE_5": {"trash": "FRI", "recycling": "FRI", "green_waste": "FRI"},
        }

    def fetch_raw_data(self) -> Any:
        """
        Fetch schedule data from Oceanside sources.

        For pilot version, use hardcoded zone data.
        In production, this would query WM API or scrape city website.
        """
        logger.info(f"Fetching data from {self.source_url}")

        # WM holiday schedule
        # If normal service day occurs on or after an observed holiday, service delayed one day that week
        return {
            "zones": self.zones,
            "holidays": [
    ...
    def parse_raw_data(self, raw_data: Any) -> ParseResult:
        ...
        try:
            ...
            for holiday in holidays:
                try:
                    ...
                    exception = ExceptionData(
                        ...
                        notes=f"Oceanside city holiday: {holiday.get('name', 'Unknown')}"
                    )
            ...
            result.errors.append(f"Failed to parse Oceanside data: {e}")
    ...
    def match_address_to_zone(self, address: str) -> str:
        ...
        In production, would use WM lookup tool or GIS polygon matching.
        ...
        # Simple heuristic based on neighborhoods/areas
        if any(kw in address_upper for kw in ["COAST", "PACIFIC", "OCEANSIDE BLVD"]):
            return "ZONE_1"
        elif any(kw in address_upper for kw in ["MISSION", "VISTA", "COLLEGE"]):
            return "ZONE_2"
        elif any(kw in address_upper for kw in ["FIRE MOUNTAIN", "RANCHO DEL ORO"]):
            return "ZONE_3"
        elif any(kw in address_upper for kw in ["ARROWOOD", "SUNSET"]):
            return "ZONE_4"
        else:
            return "ZONE_5"
