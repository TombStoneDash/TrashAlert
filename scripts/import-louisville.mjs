...
if (!SUPABASE_URL || !SUPABASE_KEY) {
  console.error('Run with: node --env-file=.env.local scripts/import-louisville.mjs')
  process.exit(1)
}
...
async function main() {
  ...
  while (true) {
    ...
    for (const f of features) {
      ...
      const day = normalizeDay(a.SRA_DAY)
      if (!day) { skipped++; continue }
      const centroid = computeCentroid(f.geometry?.rings)
      if (!centroid) { skipped++; continue }

      const route = a.SRA_ROUTE || a.SRA_GARB || a.OBJECTID
      const address = `louisville route ${String(route).toLowerCase()}`
      if (seen.has(address)) { skipped++; continue }
      seen.add(address)

      batch.push({
        address, city: 'louisville', state: 'KY', zip_code: '',
        neighborhood: `Route ${route}`,
  ...
  console.log(`\n🎉 Louisville: fetched=${fetched} imported=${imported} skipped=${skipped} errors=${errors}`)
}
