#!/usr/bin/env node
/**
 * Stevens Point WI per-address waste collection.
 * Source: services3.arcgis.com/Q3XmNaYunBtpoRGk/.../Garbage_Address_Points/FeatureServer/0
 * ~9,289 per-address points with Pickup_Da + Route_ID.
 */
...
if (!SUPABASE_URL || !SUPABASE_KEY) {
  console.error('Run with: node --env-file=.env.local scripts/import-stevens-point.mjs')
  process.exit(1)
}
const supabase = createClient(SUPABASE_URL, SUPABASE_KEY)

const BASE_URL = 'https://services3.arcgis.com/Q3XmNaYunBtpoRGk/arcgis/rest/services/Garbage_Address_Points/FeatureServer/0/query'
const FIELDS = 'Garbage_AddressPoints_Label,Garbage_AddressPoints_City,Garbage_AddressPoints_Zipcode,Garbage_AddressPoints_Pickup_Da,Garbage_AddressPoints_Route_ID,OBJECTID'
...
async function main() {
  console.log('Stevens Point WI Import')
  const reporterHash = crypto.createHash('sha256').update('city_api_stevens_point').digest('hex').substring(0, 16)
  ...
  while (true) {
    ...
    for (const f of features) {
      ...
      const day = DAY_MAP[String(a.Garbage_AddressPoints_Pickup_Da || '').trim().toLowerCase()]
      if (!day) { skipped++; continue }
      const street = String(a.Garbage_AddressPoints_Label || '').trim().toLowerCase()
      if (!street) { skipped++; continue }
      if (seen.has(street)) { skipped++; continue }
      seen.add(street)

      batch.push({
        address: street, city: 'stevens-point', state: 'WI',
        zip_code: String(a.Garbage_AddressPoints_Zipcode || '').trim(),
        neighborhood: String(a.Garbage_AddressPoints_Route_ID || '').trim(),
  ...
  console.log(`\nStevens Point: fetched=${fetched} imported=${imported} skipped=${skipped} errors=${errors}`)
}
